    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#editDialog").dialog({
                ...
                onBeforeOpen: function () {
                    ...
                    //$("#edit_portraitPath").val(selectRow.portraitPath);
                }
            });
        ...
        function uploaded() {
            ...
            if (data.success) {
                ...
                $("#add_portraitPath").val(data.portraitPath);
                $("#edit_portraitPath").val(data.portraitPath);
            } else {
...
<div id="addDialog" style="padding: 15px 0 0 55px;">
    ...
    <div style="float: right; margin: 15px 40px 0 0; width: 250px; border: 1px solid #EEF4FF" id="add-photo">
        <img id="add-portrait" alt="照片" style="max-width: 250px; max-height: 300px;" title="照片"
             src="${pageContext.request.contextPath}/static/image/portrait/default_student_portrait.png"/>
        <!-- 设置上传图片按钮 -->
    ...
    <form id="addForm" method="post" action="#">
        <table id="addTable" style="border-collapse:separate; border-spacing:0 3px;" cellpadding="6">
            <!-- 存储所上传的头像路径 -->
            <input id="add_portraitPath" type="hidden" name="portraitPath"/>
            <tr>
                <td>班级</td>
                <td colspan="1">
                    <select id="add_clazzName" style="width: 200px; height: 30px;" class="easyui-combobox"
                            name="clazzName" data-options="required:true, missingMessage:'请选择所属班级哟~'">
                        <c:forEach items="${clazzList}" var="clazz">
                            <option value="${clazz.name}">${clazz.name}</option>
                        </c:forEach>
                    </select>
            ...
            <tr>
                <td>简介</td>
                <td colspan="4"><input id="add_introduction" style="width: 200px; height: 60px;" class="easyui-textbox"
...
<div id="editDialog" style="padding: 20px 0 0 65px">
    ...
    <form id="editForm" method="post" action="#">
        ...
        <table id="editTable" style="border-collapse:separate; border-spacing:0 3px;" cellpadding="6">
            <!-- 存储所上传的头像路径 -->
            <input id="edit_portraitPath" type="hidden" name="portraitPath"/>
            <tr>
                <td>班级</td>
                <td colspan="1">
                    <select id="edit_clazzName" style="width: 200px; height: 30px;" class="easyui-combobox"
                            name="clazzName">
                        <c:forEach items="${clazzList}" var="clazz">
                            <option value="${clazz.name}">${clazz.name}</option>
                        </c:forEach>
